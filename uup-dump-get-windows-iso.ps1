...
param(
  ...
  [ValidateSet("pro", "core", "multi", "home", "server")] [string]$edition = "pro",
...
# 【已补全所有 LTSC / LTSB 及 Windows Server 目标映射】
$TARGETS = @{
  "windows-10"           = @{ search="windows 10 19045$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-10-ltsc-21h2" = @{ search="windows 10 19044$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-10-ltsc-1809" = @{ search="windows 10 17763$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-10-ltsb-1607" = @{ search="windows 10 14393$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-11-ltsc-2024" = @{ search="windows 11 26100$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-server-2025"  = @{ search="windows server 26100$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-server-2022"  = @{ search="windows server 20348$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
  "windows-server-2019"  = @{ search="windows server 17763$dotSystemRevision $arch"; edition=(Get-EditionName $edition) }
...
L4: Got langs=$($langs -join ',')."
  ...
  | Where-Object {
      ...
      $targetEdn = Get-EditionName $edition
      if ($targetEdn -eq "Multi") {
        if (($editions -notcontains "Professional") -and ($editions -notcontains "Core") -and ($editions -notcontains "Enterprise")) {
          Write-CleanLine "Skipping. Expected editions=Multi. Got editions=$($editions -join ',')."
          $res = $false
        }
      } else {
        # 兼容 Server 系列或常规 SKU 检查
        $matchFound = $false
        foreach ($ed in $editions) {
          if ($ed -like "*$targetEdn*") { $matchFound = $true; break }
        }
        if (-not $matchFound -and $editions -notcontains $targetEdn) {
          Write-CleanLine ("Skipping. Expected editions={0}. Got editions={1}." -f $targetEdn, ($editions -join ','))
          $res = $false
        }
      }

      if (!$preview -and -not ($_.Value.title -match 'version|server')) {
        Write-CleanLine "Skipping. Unexpected title format."
        $res = $false
      }
  ...
  | ForEach-Object {
      ...
      [PSCustomObject]@{
        ...
        apiUrl             = 'https://api.uupdump.net/get.php?' + (New-QueryString @{ id = $id; lang = $lang; edition = if ($edition -eq "multi") { "core;professional;enterprise" } else { $target.edition } })
        downloadUrl        = 'https://uupdump.net/download.php?' + (New-QueryString @{ id = $id; pack = $lang; edition = if ($edition -eq "multi") { "core;professional;enterprise" } else { $target.edition } })
        downloadPackageUrl = 'https://uupdump.net/get.php?' + (New-QueryString @{ id = $id; pack = $lang; edition = if ($edition -eq "multi") { "core;professional;enterprise" } else { $target.edition } })
      }
    }
}
...
function Get-WindowsIso($name, $destinationDirectory) {
  ...
  if (!$preview) {
    if ($iso.title -match 'version|server') {
      $parts = $iso.title -split '(version\s*|server\s*)'
      $verbuild = $iso.build
    } else {
      $verbuild = $iso.build
    }
